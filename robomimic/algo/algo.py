...
class Algo(object):
    ...
    def __init__(
        ...
        # for MIMO datasets
        self.frame_stack = self.global_config.train.frame_stack
    ...
    def prepare_inputs(self, obs_normalization_stats=None, **inputs):
        """
        Prepare inputs for model (like channel swap, uint8 to float conversion, normalization).
        ...
        inputs = TensorUtils.to_float(x=inputs)
        if obs_normalization_stats is not None:
            # ensure obs_normalization_stats are torch Tensors on proper device
            obs_normalization_stats = TensorUtils.to_float(x=TensorUtils.to_tensor(x=obs_normalization_stats, device=self.device))
        for obs_group in inputs:
            if obs_group in ObsUtils.OBS_GROUP_TO_KEYS.keys():
...
class RolloutPolicy(object):
    ...
    def __call__(self, **inputs):
        ...
        """
        inputs = self.policy.prepare_inputs(obs_normalization_stats=self.obs_normalization_stats, **inputs)
