...
def load_train_val_data(config):
    ...
    if config.experiment.validate:
        ...
        trainset = RobomimicDataset.factory(config=config, obs_group_to_keys=ObsUtils.OBS_GROUP_TO_KEYS, filter_by_attribute=train_filter_by_attribute)
        validset = RobomimicDataset.factory(config=config, obs_group_to_keys=ObsUtils.OBS_GROUP_TO_KEYS, filter_by_attribute=valid_filter_by_attribute)
    else:
        trainset = RobomimicDataset.factory(config=config, obs_group_to_keys=ObsUtils.OBS_GROUP_TO_KEYS, filter_by_attribute=train_filter_by_attribute)
